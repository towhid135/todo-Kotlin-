...
class UpdateUserUseCase @Inject constructor(private val repo: HomeRepo) {
    operator fun invoke(name: String, imageUrl: String): Flow<ApiResult<User>> {
        return repo.updateUser(name = name, imageUrl = imageUrl)
    }
}
